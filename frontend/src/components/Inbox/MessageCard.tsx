...
const MessageCard: React.FC<MessageProps> = (props: MessageProps) => {
  ...
  let deleteObj = {
    ...
    notification_type: props.notification.notification_type_id,
    target_id:
      props.notification.target.answer_id ||
      props.notification.target.question_id ||
      props.notification.target.subscription_id,
    is_read: props.notification.is_read,
  };
  ...
  return (
    <IonItemSliding>
      <Item lines="full" class="ion-no-padding">
        ...
        >
          ...
          <div className="cardBody">
            ...
            {props.notification.target.answer_content && (
              <IonText>
                回覆了你: {props.notification.target.answer_content}
              </IonText>
            )}
            {props.notification.target.question_content && (
              <IonText>
                提出了問題: {props.notification.target.question_content}
              </IonText>
            )}
            {props.notification.target.subscription_id && (
              <IonText>追蹤了你</IonText>
            )}
          </div>
        </MessageCardContainer>
      </Item>
      <IonItemOptions side="end">
        <IonItemOption
          onClick={() => {
            props.handleDeleteOne(
              deleteObj.notification_type,
              deleteObj.target_id
            );
            props.fetchData();
          }}
