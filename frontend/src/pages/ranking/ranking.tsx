...
const Ranking: React.FC = () => {
  const [followerCountList, setFollowerCountList] = useState(
    Array<FollowerCountList>
  );
  ...
  async function fetchFollowerCount() {
    const res = await fetch(
      `${process.env.REACT_APP_PUBLIC_URL}/user/follower/count`
    );
    if (res.ok) {
    ...
    <IonPage id="main-content">
      ...
      <IonContent>
        ...
        <Container>
            ...
            {topThree.length > 0 && (
              <div className="ranking">
                <div className="2nd" data-user_id={topThree[1].id}>
                  ...
                  {topThree[1].id === user_id ? (
                    <IonButton
                      onClick={() =>
                        router.push("/user/info", "forward", "push")
                      }
                    >
                      個人專頁
                    </IonButton>
                  ) : followingIdList.includes(topThree[1].id) ? (
                ...
                <div className="1st" data-user_id={topThree[0].id}>
                  ...
                  ) : followingIdList.includes(topThree[0].id) ? (
                    <IonButton onClick={handleUnFollowUser}>取消關注</IonButton>
                  ) : (
                    <IonButton onClick={handleFollowUser}>關注</IonButton>
                  )}
                ...
                <div className="3rd" data-user_id={topThree[2].id}>
                  ...
                  ) : followingIdList.includes(topThree[2].id) ? (
                    <IonButton onClick={handleUnFollowUser}>取消關注</IonButton>
                  ) : (
                    <IonButton onClick={handleFollowUser}>關注</IonButton>
                  )}
          ...
          <KOLRanking>
            <IonText>KOL排行</IonText>
            {otherRanking.length > 0 &&
              otherRanking.map((kol, index) => {
                return (
                  <UserCard kol={kol} key={index + 4} cardId={index + 4} />
                );
              })}
          </KOLRanking>
        </Container>
